# -*- coding: utf-8 -*-
"""
Created on Sat Jun  2 22:47:50 2018
...
if SERVER_PATHS:
    DATA_TRAIN_DIR  ="/home/shared/pizza/data/data_final_train" 
    DATA_TEST_DIR   ="/home/shared/pizza/data/data_final_test"
...
for i in r:   
    ...
    for j in range(BATCH_SIZE):
        filename=os.listdir(DATA_TRAIN_DIR)[i*BATCH_SIZE+j]
        im=plt.imread(DATA_TRAIN_DIR+"/"+filename)
        im=np.asarray(im, dtype=np.float32)
        im/=256        
        batch_input[j,:,:,:]= im[:,256:512,:]
        batch_target[j,:,:,:]=im[:,0:256,:]
...
with tf.Session() as sess:
    ...
    saver.restore(sess, MODEL_DIR+"/model_4_canny.ckpt")
    for epoch in range(EPOCHS):           
        ...
        if epoch%SAVE_EACH==0: 
            ...
            saver.save(sess,MODEL_DIR+"/model_"+str(epoch)+"_canny2"+".ckpt") 
        ...
        if epoch%TEST_EACH==0:
            ...
            for batch in batches_test:                
                ...
                img.save(DATA_OUTPUT_DIR+"/canny2_e"+str(epoch)+"_"+str(i)+'.jpg')
                i+=1